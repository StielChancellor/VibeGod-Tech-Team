...
5. **Create `VIBEGOD-STATE.md`** at the project root — the pipeline's persistent memory (see the orchestrator's Hand-offs & pipeline state protocol): objective, current stage (0), triage tier (after `/triage`), gates passed, open handovers, next action. Every later stage command updates it at its ◆ gate, so a fresh session resumes instead of re-discovering. Offer (don't force) a quick `/doctor` toolchain check now so missing tools (Playwright, graphify) surface before the build depends on them.
