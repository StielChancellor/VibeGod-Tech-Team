...
9. **GOAL acceptance-criteria certification (the DONE predicate).** For each acceptance criterion in the frozen `## GOAL` block of `VIBEGOD-STATE.md`, have **`claim-verifier`** reproduce its named proof (real agent-independent signal, not self-report), record the reproduced signal in that criterion's `verified:` slot, and flip it `[x]`. `guard-state` mechanically refuses a `[x]` flip with no `verified:` reference. **The whole-product DONE predicate = every GOAL criterion is `[x]` with recorded evidence** — the machine-checkable definition of "done" the user's final sign-off is made against. Do not present for ship until it holds (or the user explicitly waives a criterion on the record).

◆ Gate: Present the acceptance result + all gates green-with-evidence + the GOAL criteria all `[x]`-with-evidence. STOP and get the **user's explicit final go/no-go**, **then ship for end-user review.** If anything is red, fix and re-run — do not ship on a fail.
