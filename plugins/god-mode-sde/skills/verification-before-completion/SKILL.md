...
**4. Pipeline-artifact propagation:** the change is reflected through the God-Mode artifacts in order — **PRD → blueprint → code roadmap → graphify → actual code** (FLOW-SPEC Stage 9). A code change with no upstream artifact update (or vice versa) is an orphan. Any feature/journey change re-enters at the PRD stage.
